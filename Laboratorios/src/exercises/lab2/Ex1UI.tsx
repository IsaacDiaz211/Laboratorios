...
function Ex1UI() {
  const [funcStr, setFuncStr] = useState<string>("Math.PI*x*x*(9-x)/3 - 30"); 
  const [dx, setDx] = useState<number>(0.5);
  const [tol, setTol] = useState<number>(0.001);
  const [domainStart, setDomainStart] = useState<number>(-100);
  const [domainEnd, setDomainEnd] = useState<number>(100);
  ...
  const guessIntervals = (): [number, number][] => {
    ...
    for (let x = domainStart + dx; x <= domainEnd; x += dx) {
      const currF = f(x);
      if (prevF * currF <= 0) intervals.push([prevX, x]);
      prevX = x;
      prevF = currF;
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', paddingBottom: '60px' }}>
      <div style={{ maxWidth: 500, flex: 1 }}>
        <Typography.Title level={5}>
          Métodos Numéricos: Tanteo, Bisección e Interpolación
        </Typography.Title>

        <Typography.Text>Función en x:</Typography.Text>
        ...
        <Typography.Text>Tolerancia:</Typography.Text>
        <Input type="number" value={tol} onChange={(e) => setTol(parseFloat(e.target.value))} style={{ marginBottom: 12 }} />

        <Typography.Text>Rango inicio:</Typography.Text>
        <Input type="number" value={domainStart} onChange={(e) => setDomainStart(parseFloat(e.target.value))} style={{ marginBottom: 12 }} />

        <Typography.Text>Rango fin:</Typography.Text>
        <Input type="number" value={domainEnd} onChange={(e) => setDomainEnd(parseFloat(e.target.value))} style={{ marginBottom: 12 }} />

        <Button type="primary" onClick={ejecutar}>Calcular raíces</Button>
        ...
        {results.length > 0 && (
          <div style={{ marginTop: 16 }}>
            ...
            <Plot
              ...
              layout={{ title: "f(x) con raíces encontradas", xaxis: { title: "x" }, yaxis: { title: "f(x)" } }}
      ...
      <div style={{ 
        ...
        textAlign: 'center', borderTop: '1px solid #f0f0f0', 
