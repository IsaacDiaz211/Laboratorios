...
/**
 * Ejecuta desde la CLI Newton-Gregory descendente para interpolacion directa.
 * @returns Promesa que finaliza cuando la captura y la impresion concluyen.
 */
export async function runLab5Exercise2(): Promise<void> {
  console.log("Laboratorio 5 - Ejercicio 2");
  ...
  try {
    const { result, ms } = await timeExecution(() =>
      newtonGregoryBackward(xValues, yValues, xTarget)
    );
